...
print("카페로 이동합니다...")
driver.get('https://cafe.naver.com/f-e/cafes/28335242/menus/0')
time.sleep(5)

# CSV 파일에서 URL 읽기
csv_file = "swissfriends_pages_all.csv"
...
try:
    ...
    while True:
        response = supabase.table('swissfriends_post_contents').select('id').range(offset, offset + page_size - 1).execute()
...
for idx, post_info in enumerate(urls_to_crawl, 1):
    ...
    try:
        ...
        # swissfriends_content_all에 해당 ID가 있는지 확인
        try:
            check_result = supabase.table('swissfriends_content_all').select('id').eq('id', post_id).execute()
            if not check_result.data:
                print(f"  ⚠️  swissfriends_content_all에 id={post_id}가 없습니다. 스킵합니다.")
                driver.close()
                driver.switch_to.window(original_window)
                skipped_count += 1
                continue
        ...
        try:
            ...
            supabase.table('swissfriends_post_contents').upsert(content_data, on_conflict='id').execute()
            contents_saved += 1
        except Exception as e:
            print(f"  ✗ 본문 저장 실패: {e}")
        ...
        if comments_data:
            try:
                supabase.table('swissfriends_post_comments').insert(comments_data).execute()
...
print(f"  - 스킵 (ID 없음): {skipped_count}개")
print(f"  - 본문 저장: {contents_saved}개")
print(f"  - 댓글 저장: {comments_saved}개")
print(f"  - 테이블: swissfriends_post_contents, swissfriends_post_comments")
print("=" * 60)
