...
# CSV 파일 읽기
csv_input = "artbus_all.csv"
jsonl_output = "artbus_detailed.jsonl"

# 전체 처리 (테스트 시 숫자 변경)
test_count = None  # None이면 전체 처리, 숫자 지정하면 해당 개수만 처리
...
if test_count is None:
    test_count = len(articles)
    print(f"총 {len(articles)}개의 게시글을 처리합니다.\n")
else:
    print(f"총 {len(articles)}개의 게시글이 있습니다. {test_count}개만 테스트합니다.\n")

# JSONL 파일 열기 (append mode로 중간 종료 시에도 데이터 보존)
